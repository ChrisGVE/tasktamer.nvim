local M = {}
local utils = require("tasktamer.utils")
local dashboard = require("tasktamer.dashboard")
local tasks = require("tasktamer.tasks")
...
local function setup_commands()
	vim.api.nvim_create_user_command("Task2ToDo", function(opts)
		require("tasktamer").render_markdown_todos(unpack(opts.fargs))
	end, { nargs = "*" })
end
...
function M.setup(user_config)
	...
	utils.log_message("init.M.setup", "Setting up Tasktamer") -- Debug print
...
function M.initialize()
	utils.log_message("init.M.initialize", "Initializing Tasktamer") -- Debug print
	setup_commands()
end
